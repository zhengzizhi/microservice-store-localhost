...
public class InventoryServiceV1 {
	...
	@Autowired
	public InventoryServiceV1(InventoryRepository inventoryRepository, ProductRepository productRepository,
	...
	@HystrixCommand(commandKey = "productKey", commandProperties = {
			@HystrixProperty(name = "execution.isolation.thread.timeoutInMilliseconds", value = "50000") }, fallbackMethod = "getProductFallback")
	...
	public List<Inventory> getAvailableInventoryForProductIds(String productIds) {
		...
		return neo4jTemplate.loadAll(inventoryList, 1).stream().collect(Collectors.toList());
	}
}
